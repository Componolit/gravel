...
is
   ...
   function Create return Context is
     ((Types.Index'First, Types.Index'First, Types.Bit_Index'First, Types.Bit_Index'First, null, (F_Result => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final))));
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, Buffer, (F_Result => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)));
      Buffer := null;
   end Initialize;

   function Initialized (Ctx : Context) return Boolean is
     (Valid_Next (Ctx, F_Result)
      and then Available_Space (Ctx, F_Result) = (Types.Last_Bit_Index (Ctx.Buffer_Last) - Ctx.First + 1)
      and then Invalid (Ctx, F_Result));
   ...
   function Message_Last (Ctx : Context) return Types.Bit_Index is
     ((if Structural_Valid (Ctx.Cursors (F_Result)) then
       Ctx.Cursors (F_Result).Last
    else
     ...
     ((case Ctx.Cursors (Fld).Predecessor is
         when F_Initial =>
            (case Fld is
                  ...
                  when others =>
                     False),
         when F_Result | F_Final =>
            False));

   function Field_Condition (Ctx : Context; Val : Field_Dependent_Value) return Boolean is
     ((case Val.Fld is
         when F_Initial | F_Result =>
            True,
         when F_Final =>
            False));
   ...
   function Valid_Predecessor (Ctx : Context; Fld : Virtual_Field) return Boolean is
     ((case Fld is
         ...
         when F_Final =>
            (Structural_Valid (Ctx.Cursors (F_Result))
                 and Ctx.Cursors (Fld).Predecessor = F_Result)));
   ...
   begin
      ...
      case Fld is
         when F_Result =>
            Ctx.Cursors (F_Result) := (S_Invalid, Ctx.Cursors (F_Result).Predecessor);
            pragma Assert (Field_First (Ctx, Fld) = First
               and Field_Length (Ctx, Fld) = Length);
      end case;
      ...
      if Has_Buffer (Ctx)
         ...
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            if Valid_Value (Value)
               ...
               pragma Assert ((if Structural_Valid (Ctx.Cursors (F_Result)) then
                   (Ctx.Cursors (F_Result).Last - Ctx.Cursors (F_Result).First + 1) = 192
                     and then Ctx.Cursors (F_Result).Predecessor = F_Initial
                     and then Ctx.Cursors (F_Result).First = Ctx.First));
               if Fld = F_Result then
   ...
   function Structural_Valid_Message (Ctx : Context) return Boolean is
     (Structural_Valid (Ctx, F_Result));

   function Valid_Message (Ctx : Context) return Boolean is
     (Valid (Ctx, F_Result));

   function Incomplete_Message (Ctx : Context) return Boolean is
     (Incomplete (Ctx, F_Result));

   procedure Get_Result (Ctx : Context) is
      First : constant Types.Index := Types.Byte_Index (Ctx.Cursors (F_Result).First);
      Last : constant Types.Index := Types.Byte_Index (Ctx.Cursors (F_Result).Last);
   begin
      Process_Result (Ctx.Buffer.all (First .. Last));
   end Get_Result;
   ...
   procedure Set_Result (Ctx : in out Context) is
      First : constant Types.Bit_Index := Field_First (Ctx, F_Result);
      Last : constant Types.Bit_Index := Field_Last (Ctx, F_Result);
      function Buffer_First return Types.Index is
        (Types.Byte_Index (First));
      function Buffer_Last return Types.Index is
        (Types.Byte_Index (Last));
   begin
      Initialize_Result (Ctx);
      Process_Result (Ctx.Buffer.all (Buffer_First .. Buffer_Last));
   end Set_Result;

   procedure Initialize_Result (Ctx : in out Context) is
      First : constant Types.Bit_Index := Field_First (Ctx, F_Result);
      Last : constant Types.Bit_Index := Field_Last (Ctx, F_Result);
   begin
      Reset_Dependent_Fields (Ctx, F_Result);
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, Ctx.First, Last, Ctx.Buffer, Ctx.Cursors);
      pragma Assert ((if Structural_Valid (Ctx.Cursors (F_Result)) then
          (Ctx.Cursors (F_Result).Last - Ctx.Cursors (F_Result).First + 1) = 192
            and then Ctx.Cursors (F_Result).Predecessor = F_Initial
            and then Ctx.Cursors (F_Result).First = Ctx.First));
      Ctx.Cursors (F_Result) := (State => S_Structural_Valid, First => First, Last => Last, Value => (Fld => F_Result), Predecessor => Ctx.Cursors (F_Result).Predecessor);
      Ctx.Cursors (Successor (Ctx, F_Result)) := (State => S_Invalid, Predecessor => F_Result);
