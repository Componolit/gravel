package body Parpen.Protocol.Generic_Offsets with
  SPARK_Mode
is

   function Create return Context is
     ((Types.Index'First, Types.Index'First, Types.Bit_Index'First, Types.Bit_Index'First, null, (F_Data => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final))));
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, Buffer, (F_Data => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)));
      Buffer := null;
   end Initialize;

   function Initialized (Ctx : Context) return Boolean is
     (Valid_Next (Ctx, F_Data)
      and then Available_Space (Ctx, F_Data) = (Types.Last_Bit_Index (Ctx.Buffer_Last) - Ctx.First + 1)
